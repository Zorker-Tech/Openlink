...
export type TerminateTarget = {
  ...
  port: number
  /**
   * Optional host-side proxy used for the re-originated TLS connection.
   * Without this, TLS termination silently bypasses the parent proxy even
   * though opaque CONNECT traffic honours it.
   */
...
const UPSTREAM_TLS_TIMEOUT_MS = 30_000

class ExistingTlsSocketAgent extends HttpsAgent {
  private readonly upstreamSocket: TLSSocket
...
async function tlsSocketViaParentProxy(
  proxyUrl: URL,
  target: TerminateTarget,
): Promise<TLSSocket> {
  const tunnel = await connectViaParentProxy(
    proxyUrl,
    target.hostname,
    target.port,
  )
  return new Promise<TLSSocket>((resolve, reject) => {
    ...
    // openConnectTunnel intentionally resolves a paused socket so bytes after
    // the CONNECT response cannot race before a caller attaches consumers.
    // pipe() resumes it on opaque paths; tls.connect({ socket }) does not.
    tunnel.resume()
    socket.setTimeout(UPSTREAM_TLS_TIMEOUT_MS, () =>
      fail(new Error('upstream TLS handshake timed out')),
    )
...
async function forwardUpstream(
  ...
  const parentUrl =
    target.parentProxy &&
    !shouldBypassParentProxy(target.parentProxy, target.hostname)
      ? selectParentProxyUrl(target.parentProxy, { isHttps: true })
      : undefined
  let parentTlsSocket: TLSSocket | undefined
  if (parentUrl) {
    try {
      parentTlsSocket = await tlsSocketViaParentProxy(parentUrl, target)
    } catch (err) {
      logForDebugging(
        `[tls-terminate] parent proxy connection to ${target.hostname}:${target.port} failed: ${(err as Error).message}`,
        { level: 'error' },
      )
      if (!res.headersSent) {
  ...
  const upstreamAgent = parentTlsSocket
    ? new ExistingTlsSocketAgent(parentTlsSocket)
    : false
  ...
  upstream.once('close', () => upstreamAgent && upstreamAgent.destroy())
