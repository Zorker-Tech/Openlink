...
class DockerConfig(BaseModel):
    ...
    userns_mode: Optional[str] = Field(
        default=None,
        description=(
            "Optional container user namespace mode forwarded to the Docker/Podman API. "
            "Use 'keep-id' for rootless Podman so bind-mounted Project VM paths retain "
            "their guest ownership; leave unset for Docker/rootful runtimes."
        ),
    )
    security_opts: list[str] = Field(
        default_factory=list,
        description=(
            "Additional raw Docker/Podman security options applied to sandbox containers. "
            "For a SELinux-labelled Project VM bind mount, use ['label=disable'] while "
            "retaining the Project VM as the isolation boundary."
        ),
    )
