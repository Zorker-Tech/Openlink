...
class DockerNetworkingMixin:
    ...
    def get_endpoint(self, sandbox_id: str, port: int, resolve_internal: bool = False,
        ...
        if resolve_internal:
            ...
            # main container's private IP is the sidecar's bridge IP. Use that shared namespace
            # address for server-side proxying: a rootful Podman VM cannot reliably hairpin from
            # the OpenSandbox server through host.containers.internal to a freshly published
            # high port, even though the published endpoint is reachable from outside the VM.
    ...
    def _start_egress_sidecar(
        ...
        base_sidecar_host_config_kwargs: dict[str, Any] = {
            ...
            "cap_add": ["NET_ADMIN"],
            # The sidecar must return with its Agent container after a Project
            # VM reboot so provider traffic never races a missing egress path.
    ...
    def _wait_for_egress_sidecar_ready(
        ...
        def probe_sidecar_loopback() -> bool:
            """Probe the sidecar directly through the Docker exec API.

            Rootful Podman machines publish bridge ports on the Linux VM.  The
            OpenSandbox server is itself running in that VM, so resolving the
            published port through ``host.containers.internal`` can fail due to
            hairpin routing even while the sidecar is healthy.  An exec probe
            exercises the actual readiness endpoint without that extra network
            hop and works for both Docker and Podman backends.
            """
            if sidecar_container is None:
                return False
            try:
                result = sidecar_container.exec_run([
                    "/usr/bin/curl",
                    "--fail",
                    "--silent",
                    "--show-error",
                    ...
                    "1",
                    "-H",
                    f"OPENSANDBOX-EGRESS-AUTH: {egress_token}",
                ...
                last_error_detail = detail.strip() or f"exit code {exit_code}"
                logger.warning(
                    "sandbox=%s | egress loopback probe not ready: %s",
                    sandbox_id,
                    last_error_detail,
                )
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    "sandbox=%s | egress loopback probe failed: %s",
                    sandbox_id,
                    exc,
                )
            return False
        ...
        while time.monotonic() < deadline:
            # Prefer the direct probe.  This is the reliable path for a
            # server running inside a rootful Podman VM and avoids waiting for
            # host-port hairpin routing to converge.
            if probe_sidecar_loopback():
                return
            ...
            # Podman publishes bridge ports on the VM host, but the
            # server-side host-gateway address is not guaranteed to expose a
            # freshly allocated high port immediately (this is common with
            # rootful Podman machines). Probe the sidecar's own loopback as a
            # reliable fallback; this validates the same health gate without
            # depending on host-port hairpin routing.
            if probe_sidecar_loopback():
                return
            time.sleep(0.2)
        raise HTTPException(
