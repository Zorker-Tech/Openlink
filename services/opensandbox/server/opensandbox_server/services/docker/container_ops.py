...
class DockerContainerOpsMixin:
    ...
    def _base_host_config_kwargs(
        ...
        # OpenLink sandboxes are durable Project-VM workloads. Keep the Agent
        # process resident across a VM/host reboot; explicit timeout cleanup
        # and DELETE still remove the container normally.
        host_config_kwargs: Dict[str, Any] = {
            "network_mode": network_mode,
            "restart_policy": {"Name": "unless-stopped"},
        }
        docker_cfg = self.app_config.docker
        security_opts: list[str] = list(docker_cfg.security_opts)
        if docker_cfg.userns_mode:
            # Podman exposes Docker's HostConfig.UsernsMode field.  In a
            # rootless Project VM, keep-id maps the guest user into the
            # sandbox so 0700 project/session bind mounts remain private while
            # still being usable by the worker container.
            host_config_kwargs["userns_mode"] = docker_cfg.userns_mode
    ...
    def _create_and_start_container(
        ...
        # docker-py 7.1.0 only accepts ``userns_mode="host"`` even though the
        # Docker API (and Podman compatibility socket) supports additional
        # values such as ``keep-id``.  Keep the value in the internal kwargs
        # until this boundary, then inject the API field after docker-py has
        # performed its validation.  This preserves Podman's rootless
        # user-namespace mapping without weakening validation for every other
        # HostConfig option.
        userns_mode = host_config_kwargs.pop("userns_mode", None)
