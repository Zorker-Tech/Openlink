...
def test_docker_userns_mode_is_forwarded_for_rootless_podman():
    config = DockerConfig(userns_mode="keep-id")
...
async def test_create_sandbox_applies_security_defaults(mock_docker):
    ...
    assert mock_client.api.create_host_config.call_args.kwargs["restart_policy"] == {
        "Name": "unless-stopped"
    }
    assert "label=disable" in host_config.get("security_opt", [])
...
@pytest.mark.asyncio
@patch("opensandbox_server.services.docker.docker_service.docker")
async def test_create_sandbox_passes_userns_mode(mock_docker):
    mock_client = MagicMock()
    mock_client.containers.list.return_value = []
    mock_client.api.create_host_config.return_value = {}
    mock_client.api.create_container.return_value = {"Id": "cid"}
    ...
    cfg = _app_config()
    cfg.docker.userns_mode = "keep-id"
    ...
    assert "userns_mode" not in host_config_kwargs
    host_config = mock_client.api.create_container.call_args.kwargs["host_config"]
    assert host_config.get("UsernsMode") == "keep-id"
...
async def test_egress_sidecar_injection_and_capabilities(mock_docker):
    ...
    assert sidecar_kwargs["host_config"]["restart_policy"] == {"Name": "unless-stopped"}
    ...
    assert main_kwargs["host_config"]["restart_policy"] == {"Name": "unless-stopped"}
