...
| `userns_mode` | string \| omitted | `null` | Optional Docker/Podman user namespace mode. OpenLink's rootless Project VM uses **`keep-id`**. |
| `security_opts` | list of strings | `[]` | Additional raw Docker/Podman security options. OpenLink's SELinux-labelled Project VM uses `['label=disable']` for bind-mounted session paths. |
