 ...
 * 64 (chat-avatar scale), 30 (status-capsule scale), and 20
 * (inline-text scale). Each size carries its own dot count, dot size
 * and speed tuning — they are separate designs, not a scale factor.
 */
export type OrbSize = 64 | 30 | 20;
...
export interface ThinkingOrbProps extends Omit<CanvasHTMLAttributes<HTMLCanvasElement>, 'style'> {
  ...
  /** Tuned size preset — 64, 30, or 20 CSS px. @default 64 */
  size?: OrbSize;
