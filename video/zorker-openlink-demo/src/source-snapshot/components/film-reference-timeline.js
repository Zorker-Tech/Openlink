...
import { ThinkingOrb } from "./thinking/src/index.js";
...
function dynamicTaskLabel(executions, active, t) {
    ...
    return [...actions].map(([action, count]) => {
        ...
        if (action === 'edit')
            return active ? t('正在编辑 {target}', { target: count === 1 ? t('文件') : t('{count} 个文件', { count }) }) : t('Edited {target}', { target: target('a file', 'files') });
        if (action === 'read')
...
function latestFlowForItems(items, t) {
    ...
    for (const item of [...items].reverse()) {
        if (item.kind === 'message')
            return { label: item.streaming ? t('正在生成回复') : t('生成回复'), state: 'composing' };
        if (item.kind === 'reasoning')
            return { label: item.streaming ? t('正在思考') : t('完成思考'), state: 'solving' };
        if (item.kind === 'status')
...
function getActiveFlowStatus(timeline, t) {
    ...
    for (const item of [...timeline.items].reverse()) {
        ...
        if (item.kind === 'message' && item.streaming)
            return { label: t('正在生成回复'), state: 'composing' };
        if (item.kind === 'reasoning' && item.streaming)
            return { label: t('正在思考'), state: 'solving' };
...
function TaskPositionCapsule({ activeFlow }) {
    ...
    if (!awayFromTaskPosition || !escapedFromLock)
        return null;
    return (_jsxs("button", { "aria-label": activeFlow ? t('返回正在工作的位置') : t('返回任务位置'), className: "absolute bottom-3 left-1/2 z-30 flex h-10 max-w-[calc(100%_-_24px)] -translate-x-1/2 items-center gap-1.5 overflow-hidden rounded-full border border-[var(--app-status-glass-border)] bg-[var(--app-status-glass)] py-1 pl-1.5 pr-3 text-left text-[var(--app-status-glass-foreground)] shadow-[inset_0_0_50px_rgb(255_255_255_/_1%),0_16px_48px_var(--app-shadow)] backdrop-blur-xl transition-transform active:scale-[0.98]", onClick: () => scrollToBottom(), type: "button", children: [_jsx(ThinkingOrb, { "aria-label": capsuleFlow.label, className: "shrink-0", size: CAPSULE_ORB_SIZE, speed: activeFlow ? 1 : 0.5, state: capsuleFlow.state }), _jsx("span", { className: "min-w-0 max-w-[160px] truncate text-[12px] font-normal leading-[14px]", children: activeFlow?.label ?? t('空闲中') })] }));
}
...
function EditableUserMessage({ item, disabled, onEditMessage }) {
    ...
    const submit = async () => {
        if (pending || disabled || !draft.trim() || !onEditMessage)
            return;
        setPending(true);
        setError(null);
        try {
            await onEditMessage(item.id, draft.trim());
            setEditing(false);
        }
        catch (cause) {
            setError(cause instanceof Error ? cause.message : t('重发失败，修改内容已保留'));
        }
        finally {
            setPending(false);
        }
    };
    return _jsxs("div", { className: "relative flex w-full justify-end", "data-message-id": item.id, children: [editing && _jsx("div", { "aria-hidden": "true", className: "invisible mb-4 max-w-[95%] whitespace-pre-wrap break-words rounded-xl border px-3 py-1.5 text-[13px] leading-[21.125px]", children: item.text }), editing ? _jsxs("form", { "aria-label": t('编辑原消息'), className: "absolute left-1/2 top-0 z-20 mx-auto flex w-[95%] min-w-0 -translate-x-1/2 flex-col gap-3 rounded-xl border border-dashed border-[var(--app-muted)] bg-[var(--app-elevated)] p-3 shadow-[0_12px_36px_var(--app-shadow)]", onSubmit: (event) => { event.preventDefault(); void submit(); }, onKeyDown: (event) => {
                ...
                }, children: [_jsx("textarea", { autoFocus: true, "aria-label": t('消息内容'), className: "min-h-20 max-h-64 w-full resize-y bg-transparent text-[13px] leading-[21px] text-[var(--app-foreground)] outline-none", value: draft, maxLength: 10000, disabled: pending, onChange: (event) => setDraft(event.target.value) }), _jsx("p", { className: "text-[11px] leading-4 text-[var(--app-muted)]", children: t('从此处重新生成后续对话，不回滚文件修改。') }), error && _jsx("p", { role: "alert", className: "text-xs text-[var(--app-danger)]", children: error }), _jsxs("div", { className: "flex justify-end gap-2", children: [_jsx("button", { type: "button", className: "rounded-md px-3 py-1.5 text-xs hover:bg-[var(--app-hover)] focus-visible:outline", disabled: pending, onClick: cancel, children: t('取消') }), _jsx("button", { type: "submit", className: "rounded-md bg-[var(--app-submit-background)] px-3 py-1.5 text-xs text-[var(--app-submit-foreground)] disabled:opacity-40 focus-visible:outline", disabled: pending || disabled || !draft.trim(), children: pending ? t('正在重发…') : t('重发') })] })] }) : _jsx("button", { ref: trigger, type: "button", "aria-label": t('编辑消息：{text}', { text: item.text }), disabled: disabled || !onEditMessage, className: "mb-4 max-w-[95%] whitespace-pre-wrap break-words rounded-xl border border-[var(--app-control-border)] bg-[var(--app-active)] px-3 py-1.5 text-left text-[13px] leading-[21.125px] text-[var(--app-foreground)] outline-none hover:border-[var(--app-muted)] focus-visible:ring-1 focus-visible:ring-[var(--app-muted)]", onClick: () => { setDraft(item.text); setError(null); setEditing(true); }, children: item.text })] });
}
...
function StreamingThinkingItem() {
    const t = useT();
    return (_jsxs("div", { className: "flex h-7 items-center gap-2 text-[13px] leading-[21px] text-[var(--app-muted)]", "data-streaming-thinking": true, children: [_jsx("span", { className: "flex size-5 shrink-0 items-center justify-center", children: _jsx(ThinkingOrb, { "aria-label": t('正在思考'), className: "shrink-0", size: 20, state: "solving" }) }), _jsx("span", { children: t('Thinking…') })] }));
}
...
export function ChatTimeline({ events, isStreaming = false, showThinkingContent = true, onFlowStatusChange, onEditMessage, }) {
    ...
    const activeFlow = useMemo(() => getActiveFlowStatus(timeline, t) ?? (isStreaming ? { label: t('正在思考'), state: 'solving' } : null), [isStreaming, timeline]);
    useEffect(() => {
        onFlowStatusChange?.(activeFlow);
    }, [activeFlow, onFlowStatusChange]);
    return (_jsxs(Conversation, { className: "min-h-0 flex-1", children: [_jsxs(ConversationContent, { className: "min-h-full gap-5 px-3 pb-6 pt-20 text-[13px] leading-[21.125px] tracking-[-0.0762px]", children: [rounds.map((round) => (_jsxs("section", { className: "flex w-full flex-col gap-3", "data-round-state": round.state, children: [round.user ? _jsx(EditableUserMessage, { item: round.user, disabled: isStreaming, onEditMessage: onEditMessage }) : null, round.thinkingItems.map((item) => _jsx(TimelineItemView, { item: item, showContent: showThinkingContent }, item.id)), isStreaming && round === rounds.at(-1) && !round.thinkingItems.length && !round.workingItems.length && !round.outputItems.length && !round.ending ? _jsx(StreamingThinkingItem, {}) : null, round.workingItems.length || round.checkpoint ? (_jsx(WorkFlowGroup, { activeFlow: round.state === 'working' ? activeFlow : null, round: round, children: round.workingItems.map((item) => _jsx(TimelineItemView, { item: item, showContent: showThinkingContent }, item.id)) })) : null, round.outputItems.map((item) => _jsx(TimelineMessageItem, { item: item }, item.id)), _jsx(EndingSection, { round: round })] }, round.id))), _jsx("div", { className: "h-px w-full", "data-task-position": "current" })] }), _jsx(TaskPositionCapsule, { activeFlow: activeFlow })] }));
}
