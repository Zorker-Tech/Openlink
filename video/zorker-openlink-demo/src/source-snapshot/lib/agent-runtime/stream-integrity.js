export function missingTerminalEventMessage(clientDisconnected) {
...
export function isTerminalErrorEvent(event) {
 ...
 */
export function orderAgentEvents(events) {
    let sorted = true;
 ...
 */
export function recoverInterruptedInitialEvents(events, now = new Date(), minimumAgeMs = 10_000) {
    const lastUserIndex = events.findLastIndex((event) => event.type === 'message.user');
    if (lastUserIndex < 0)
        return events;
    const turnEvents = events.slice(lastUserIndex + 1);
    if (!turnEvents.some((event) => event.type === 'session.started'))
        return events;
    if (turnEvents.some((event) => event.type === 'session.completed' || isTerminalErrorEvent(event)))
        return events;
    const lastEvent = events.at(-1);
    if (!lastEvent)
        return events;
    const lastEventTime = Date.parse(lastEvent.timestamp);
    if (Number.isFinite(lastEventTime) && now.getTime() - lastEventTime < minimumAgeMs)
        return events;
