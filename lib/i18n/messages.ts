import { DEFAULT_LOCALE, isLocale, normalizeLocale, type Locale } from './locales'

/**
 * Translation catalog: source string (Simplified Chinese) → localized string.
 *
 * Keys are the authored zh-CN text. Catalogs may use `{name}` placeholders,
 * which `translate` fills from the values passed at the call site.
 */
...
export function translate(messages: Messages | null | undefined, source: string, values?: Record<string, string | number>): string {
...
export function readLocaleCookieValue(value: string | undefined | null): Locale {
...
export function createTranslator(messages: Messages) {
  return (source: string, values?: Record<string, string | number>) => translate(messages, source, values)
}
